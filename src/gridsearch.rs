//! Single-room weighted grid search — a general pathfinding primitive.
//!
//! Engine `PathFinder` cost matrices are u8 (1..=254, 255 = impassable): they
//! cannot express searches whose tile weights span orders of magnitude — e.g.
//! breach planning, where entering a tile costs its blockers' hit pool
//! (hundreds to millions). Searches of that class live HERE, in the pathfinding
//! system, and are consumed by feature modules as *pricing policy* — never as
//! bespoke algorithms in feature code (operator directive 2026-06-13; EP-2.6
//! one implementation per concern). Engine-walkable navigation stays on the
//! rover movement system / `PathFinder`.
//!
//! Pure `std` (no `screeps`/JS): the room is a 50×50 grid of `(u8, u8)` tiles,
//! costs/passability arrive as closures, so both the live bot and the headless
//! combat sim use the one implementation.
...
/// Deterministic per EP-6.13: the heap orders by (cost, tile index), so
/// equal-cost routes always resolve the same way.
